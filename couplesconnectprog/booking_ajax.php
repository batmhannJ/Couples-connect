...
if($_POST['event_action'] == "first_load" || $_POST['event_action'] == "changeDate" ||  $_POST['event_action'] == "cancel_booking"){
        ...
        $ret['html'].= "<tr>";
            ...
            $ret['html'].= "<td>";
                $ret['html'].= "<select class='form-control w-75 ms-2 select_date'>";
                    ...
                    mf_appointment_info.userid = mf_prog_users.userid LEFT JOIN mf_venue ON ext_appointment_info.venue_id = mf_venue.venue_id WHERE ext_appointment_info.venue_id='".$row_xid['venue_id']."' AND mf_appointment_info.sched_type='".$act_status2."' GROUP BY ext_appointment_info.clinic_date";
            ...
            if($_POST['event_action'] == 'first_load'){
                ...
                if($_POST['event_action'] == 'changeDate'){
                    ...
                    WHERE ext_appointment_info.venue_id='".$row_xid['venue_id']."'  AND mf_appointment_info.sched_type='".$act_status2."' AND ext_appointment_info.clinic_date='".$_POST['date']."'  ORDER BY time_from ASC LIMIT 1";
                ...
                if($_POST['ext_recid'] == $row_xid['ext_recid']){
                    ...
                    $ret['html'].= "<td>";
                        ...
                        ext_appointment_info.appointment_info_id = mf_appointment_info.appointment_info_id LEFT JOIN mf_venue ON ext_appointment_info.venue_id = mf_venue.venue_id
                        WHERE ext_appointment_info.venue_id='".$row_xid['venue_id']."'  AND mf_appointment_info.sched_type='".$act_status2."' AND ext_appointment_info.clinic_date='".$_POST['date']."'  ORDER BY time_from ASC";
    ...
    while($row_xid = $stmt->fetch()){
        ...
        $ret['html'].= "<tr>";
            $ret['html'].= "<td style='padding-bottom:15px;padding-top:10px'>";
                $ret['html'].= "<div class='container text-start' style='font-family:inter;font-weight:700;font-size:25px;color:black;padding-left:25px'>";    
                    if(($_POST['ext_recid'] == $row_xid['ext_recid'])  && ($row_xid['is_online']) == 'Y'){
                        $ret['html'].= "<a target='_blank' style='color:blue!important;text-decoration:underline!important' href='".$row_xid["venue_link"]."'>".$row_xid["venue"]."</a>";   
                    }else{
                        $ret['html'].= $row_xid["venue"];    
                    }
            ...
            $ret['html'].= "<td>";
                $ret['html'].= "<select class='form-control w-75 ms-2 select_date' disabled>";
                    ...
                    WHERE ext_appointment_info.venue_id='".$row_xid['venue_id']."' AND mf_appointment_info.sched_type='".$act_status2."'";
                    $stmt_2	= $link->prepare($select_2);
                    $stmt_2->execute();

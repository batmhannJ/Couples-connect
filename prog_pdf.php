<?php
    ...
    if ($_POST['txt_output_type']=='tab')
	{
        ...
        // $xfilename  =   $_POST["main_header_hidden"]."_".$date_today."_pdf";
        
        // $pdf->ezStream("xls",$xfilename);
	}
	else
	{
        ...
        $xreport_title = $_POST["main_header_hidden"]." PDF";
        $pdf->addInfo('Title',$xreport_title);
	}
	$pdf ->selectFont("ezpdfclass/fonts/Helvetica.afm");
    ...
    $fields.=",recid";
        ...
        if((isset($_POST["search_hidden_value"]) && $_POST["search_hidden_value"] !== "") && isset($_POST['search_hidden_dd'])){
            $pdf->ezPlaceData($xleft, $xtop,$_POST['search_hidden_dd'].":", 9, 'left' );
            $header_search_value = '';

            if(($search_text_input !== "")){
                $header_search_value = $search_text_input;
            }else{
                $header_search_value = $_POST['search_hidden_value'];
            }

            if($_POST['search_hidden_type'] == "checkbox"){
                if($header_search_value === 1){
                    $header_search_value = "Checked";
                }
                else if($header_search_value === 0){
                    $header_search_value = "Unchecked";
                }
                else{
                    $header_search_value = "All";
                }

            }
            $pdf->ezPlaceData(dynamic_width($_POST['search_hidden_dd'].":",$xleft,3,'cus_left'), $xtop,"<b>".$header_search_value."</b>", 9, 'left' );
            $xtop   -= 15;
        }
        ...
        foreach($_POST["fields"] as $fields_arr => $fields_arr_val){
            ...
            if ($_POST['txt_output_type']=='tab')
            {
                $fields_arr_val["fheader"] = $fields_arr_val["fheader"];
            }else{
                $fields_arr_val["fheader"] = trim_str($fields_arr_val["fheader"],100,10);
            }

            if($xfields_heaeder_counter == 0){

                $pdf->ezPlaceData($xleft,$xtop,"<b>".$fields_arr_val["fheader"]."</b>",10,'left');
            }else{
                $pdf->ezPlaceData($xleft+=115,$xtop,"<b>".$fields_arr_val["fheader"]."</b>",10,'left');
            }
    ...
    while($rs_main = $stmt_main->fetch()){    
        ...
        foreach($_POST["fields"] as $fields_arr => $fields_arr_val){
            ...
            if ($_POST['txt_output_type']=='tab')
            {
                $rs_main[$fields_arr_val["fname"]] = $rs_main[$fields_arr_val["fname"]];
            }else{
                $rs_main[$fields_arr_val["fname"]] = trim_str($rs_main[$fields_arr_val["fname"]],$xmaxwidth, 9);
            }


        
            if($fields_count_data == 0){
                $pdf->ezPlaceData(dynamic_width($fields_arr_val["fheader"],25,$x_add_space,$xalign),$xtop,$rs_main[$fields_arr_val["fname"]],9,$xalign);
            }else{
                $pdf->ezPlaceData(dynamic_width($fields_arr_val["fheader"],$xleft+=115,$x_add_space,$xalign),$xtop,$rs_main[$fields_arr_val["fname"]],9,$xalign);
            }
